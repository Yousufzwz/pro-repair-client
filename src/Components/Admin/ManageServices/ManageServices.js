...
const ManageServices = () => {
    ...
    useEffect(()=>{
        fetch("https://blooming-reef-97776.herokuapp.com/getCustomerOrder") 
        .then(res => res.json())
        .then(data => {
            setOrderList(data)
        })
                            ...
                            <tbody>
                                {
                                 ...
                                 <tr>
                                    ...
                                    <td>{data.Price}</td>
                                    <td><small style={{backgroundColor:'#e5ed9d'}}>Pending</small></td>
                                    <td><button className="btn btn-danger mt-4" style={{marginBottom:'100px'}}>Delete</button></td>
